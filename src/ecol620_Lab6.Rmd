...
if(
  ...
  ###########################################
    commute_dist_matrix = gdistance::commuteDistance(
      transition_layer
      , public_centroids
    )
  ###########################################
...
###########################################
  ...
  # create data frame of correlation matrix
    euclidean_dist_df <- euclidean_dist_matrix %>% 
      as.data.frame() %>% 
      tibble::rownames_to_column(var = "public_lands_pt1") %>% 
      tidyr::pivot_longer(
...
if(
  gdistance_pkg_still_requires_raster_pkg_format==FALSE
  | raster_pkg_functions_work_on_my_computer==FALSE
){
least_cost_matrix <- c(
      250607.85, NA, NA, NA
